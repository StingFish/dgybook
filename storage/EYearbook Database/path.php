...
<head>
<meta charset="utf-8">
<title>Employee's Yearbook Table</title>
...
.pic{
  ...
  margin-top:10px;
  margin-bottom: 0;
  border-radius: 10px;box-shadow: rgba(0, 0, 0, 0.25) 0px 54px 55px, rgba(0, 0, 0, 0.12) 0px -12px 30px, rgba(0, 0, 0, 0.12) 0px 4px 6px, rgba(0, 0, 0, 0.17) 0px 12px 13px, rgba(0, 0, 0, 0.09) 0px -3px 5px;
}
...
<header>
  ...
  <p><a class="un" href="../../storage/EYearbook Database/path.php" style="float: left;font-size: 15px;display: block;color: white;">Employee's Yearbook Table</a></p>
...
<div id="myModal" class="modal">
  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
        ...
        Data Year:
        <?php $goose=date("Y"); ?>
      <input type="number" id="num" min="2018" max="<?php echo $goose; ?>" name="f1" style="width:100%;border-bottom:1px solid black;outline:none;" onkeypress="return /[0-9]/i.test(event.key)">
    </div>
    ...
    if(move_uploaded_file($_FILES["file1"]["tmp_name"], $target_file)){
      ...
      $user_check_query = "SELECT * FROM folder2 where year='$yr' AND type='A' LIMIT 1";
         ...
         if ($user) { // if user exists
            ...
            $checkSameYear = "SELECT * FROM folder2 WHERE year = '$yr' AND type = 'B' LIMIT 1";
            $ress = mysqli_query($db, $checkSameYear);
            $count = mysqli_fetch_array($ress);
            if($count){
              $adds="INSERT INTO folder2 (frontImage, type, year) VALUES ('$bars','A', '$yr')";
              $alter1 = "UPDATE folder2 SET frontImage = '$bars' WHERE type = 'B' AND year = '$yr'";
              mysqli_query($db, $adds);
              mysqli_query($db, $alter1);
              echo "<script>window.location='path.php';</script>";
            }else{
              $insert2="INSERT INTO folder2 (frontImage, type, year) VALUES ('$bars','A', '$yr'), ('$bars','B', '$yr')";
...
if (isset($_POST['submit2'])) {
    ...
    $yr= mysqli_real_escape_string($db, $_POST['f1']);
         ...
         $user_check_query = "DELETE FROM folder2 WHERE year='$yr' AND type='A'";
         $result = mysqli_query($db, $user_check_query);
         echo "<script>window.location='path.php';</script>";
}
